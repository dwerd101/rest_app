...
as
select profile_result.id as prof_id, field_id, date_field, domain, comment,
       profile_task_id, profile_task.id as task_id, source_id, create_date, update_date, status
from profile_result inner join profile_task on profile_result.profile_task_id = profile_task.id

create view profileTaskView  as
select profile_task_id, field_id,  owner.name as  owner_name, "table".name as table_name,  field_name, domain, comment
